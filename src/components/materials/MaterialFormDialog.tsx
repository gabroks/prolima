...
import { DiscardAlertDialog } from "@/components/shared/DiscardAlertDialog";
...
function isFormDirty(form: MaterialForm): boolean {
  return !!(form.name || form.basePrice || form.notes || form.category);
}

export function MaterialFormDialog({
  open, onOpenChange, form, onFormChange, onSave, isEditing, isPending, categories,
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={onSave} disabled={isPending}>
              {isPending ? <><Loader2 className="h-4 w-4 mr-1.5 animate-spin" />{isEditing ? "Atualizando…" : "Salvando…"}</> : isEditing ? "Atualizar" : "Salvar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <DiscardAlertDialog open={discardOpen} onCancel={() => setDiscardOpen(false)} onConfirm={() => { setDiscardOpen(false); onOpenChange(false); }} />
    </>
  );
}
