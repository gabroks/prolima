      ...
      <div className="flex items-center justify-between pb-6">
        ...
        <div className="flex gap-2">
          {step < 3 ? <Button onClick={() => setStep(s => Math.min(3, s + 1))} disabled={!canProceed(step)}>Próximo<ChevronRight className="h-4 w-4 ml-1.5" /></Button> : (<>
            <Button variant="outline" onClick={() => handleSave(true)}><Save className="h-4 w-4 mr-1.5" />Salvar Rascunho</Button>
            <Button onClick={() => handleSave(false)} className="shadow-md shadow-primary/20"><Send className="h-4 w-4 mr-1.5" />Emitir Orçamento</Button>
          </>)}
        </div>
      </div>
