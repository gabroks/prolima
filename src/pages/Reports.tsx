...
export default function Reports() {
  ...
  const topClients = useMemo(() => {
    ...
    budgets.forEach(b => { if (!map[b.client_id || b.client_name]) map[b.client_id || b.client_name] = { name: b.client_name, value: 0, count: 0, paid: 0 }; map[b.client_id || b.client_name].value += Number(b.total); map[b.client_id || b.client_name].count++; });
    payments.forEach(p => { const budget = budgets.find(b => b.id === p.budget_id); if (budget) { const key = budget.client_id || budget.client_name; if (map[key]) map[key].paid += Number(p.amount); } });
    return Object.values(map).sort((a, b) => b.value - a.value).slice(0, 5);
  }, [budgets, payments]);

  const statusData = useMemo(() => (Object.keys(budgetStatusConfig) as BudgetStatus[]).map(key => ({ name: budgetStatusConfig[key].label, value: budgets.filter(b => b.status === key).length })).filter(d => d.value > 0), [budgets]);
  ...
  const ticketMedio = budgets.length > 0 ? totalBudgetValue / budgets.length : 0;
