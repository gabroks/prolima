...
export default function Financial() {
  ...
  const openNew = () => {
    setEditingId(null);
    setForm({ method: "PIX", date: new Date().toISOString().split("T")[0] });
    setDialogOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    const fullForm: PaymentForm = {
      ...
      clientName: budget?.client_name || form.clientName || "",
      amount: form.amount!,
    ...
    if (editingId) {
      updatePayment.mutate({ id: editingId, form: fullForm }, { onSuccess });
    } else {
      createPayment.mutate(fullForm, { onSuccess });
    }
  };

  const handleDelete = () => {
    if (!deleteId) return;
    deletePaymentMut.mutate(deleteId, { onSuccess: () => setDeleteId(null) });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <span className="text-xs text-muted-foreground">
          ...
          {methodFilter !== "all" && <> • <span className="font-medium text-foreground">{methodFilter}</span></>}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {(() => {
                const payment = deleteId ? payments.find(p => p.id === deleteId) : null;
                if (payment) {
                  return `${payment.client_name} — ${formatCurrency(Number(payment.amount))} via ${payment.method}. Esta ação não pode ser desfeita.`;
                }
